...
export default function Orb({
  ...
  useEffect(() => {
    ...
    if (!supportsWebGL) {
      reportError("WebGL is not available");
      return;
    }
    ...
    try {
      ...
      reportError(error instanceof Error ? error.message : "Orb renderer failed");
      return;
    }
    ...
    const update = (t: number) => {
      ...
      const effectiveHover = forceHoverState ? 1 : enableHover ? targetHover : 0;
      program.uniforms.hover.value += (effectiveHover - program.uniforms.hover.value) * 0.1;

      const speedBoost = rotateOnHover && effectiveHover > 0.5 ? hoverRotationBoost : 0;
      currentRot += dt * (baseRotationSpeed + speedBoost);
